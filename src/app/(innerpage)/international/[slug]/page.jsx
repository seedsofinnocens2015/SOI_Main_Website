...
// Helper function to convert center name to international slug
// Example: "Lusaka, Zambia, Africa" -> "lusaka-zambia"
// Example: "Kitwe, Zambia, Africa" -> "kitwe-zambia"
// Example: "Mabela, Muscat, Oman" -> "mabela-muscat"
function getInternationalSlug(centerName) {
  const parts = centerName.split(',').map(p => p.trim());
  // Take first two parts (city and country), skip third part (continent/region)
  const cityCountry = parts.slice(0, 2).join('-');
  return cityCountry
    .toLowerCase()
    .replace(/\s+/g, '-')
    .replace(/[^a-z0-9-]/g, '');
...
export async function generateStaticParams() {
  try {
    ...
    return centresData
      ...
      .map((center) => {
        const cityCountrySlug = getInternationalSlug(center.name);
        return {
          slug: cityCountrySlug,
        };
      })
...
const page = async ({ params }) => {
  ...
  // Find center by matching the international slug format
  const center = centresData.find((c) => {
    if (!c.isInternational) {
      return false;
    }

    const centerSlug = getInternationalSlug(c.name);
    return centerSlug === slug;
  });
  ...
  const rawCenterContent = { ...rawCenterContentData };

  // Convert for IVFContentSection (excluding first section)
  const centerContentData = {
    sections: rawCenterContentData.sections.slice(1)
  };

  const firstSection = rawCenterContentData.sections[0];
