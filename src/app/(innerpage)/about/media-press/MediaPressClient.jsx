...
const MediaPressClient = () => {
  ...
  return (
    <div>
          ...
          <div className="row cs_gap_y_30" style={{ gap: '30px 0' }}>
            {youtubeVideosData.map((video, index) => (
              <div key={index} className="col-lg-6 col-md-6">
                <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0px 2px 10px rgba(0, 0, 0, 0.05)', border: '1px solid #e8e8e8', transition: 'all 0.3s ease', height: '100%', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ position: 'relative', width: '100%', height: '300px', overflow: 'hidden', cursor: 'pointer' }} onClick={() => openModal(video)}>
                    <Image src={video.thumbnail} alt={video.title} width={800} height={450} style={{ width: '100%', height: '100%', objectFit: 'cover' }} unoptimized />
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '68px', height: '48px', backgroundColor: 'rgb(248, 1, 1)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M8 5v14l11-7z" /></svg>
                    </div>
                  </div>
                  <div style={{ padding: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                      <span style={{ fontSize: '14px', color: '#df3655', fontWeight: '500' }}>{video.date}</span>
                      <span style={{ fontSize: '12px', color: '#666', fontWeight: '400' }}>{video.channel}</span>
                    </div>
                    <h4 style={{ fontSize: '18px', fontWeight: '600', color: '#000000', marginBottom: '10px', lineHeight: '1.4', cursor: 'pointer' }} onClick={() => window.open(video.youtubeUrl, '_blank')}>
                      {video.title}
                    </h4>
                    <div>
                      <p style={{ fontSize: '14px', color: '#666', lineHeight: '1.6', margin: 0, marginBottom: '8px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {video.description}
                      </p>
                      <button onClick={() => openModal(video)} style={{ background: 'none', border: 'none', color: '#df3655', fontSize: '14px', fontWeight: '600', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}>
                        View More
                      </button>
                    </div>
                  </div>
      ...
      {selectedVideo && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.8)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', overflowY: 'auto' }} onClick={closeModal}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', maxWidth: '900px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative', boxShadow: '0px 10px 40px rgba(0, 0, 0, 0.3)' }} onClick={(e) => e.stopPropagation()}>
            <button onClick={closeModal} style={{ position: 'absolute', top: '15px', right: '15px', background: 'rgba(0, 0, 0, 0.7)', border: 'none', borderRadius: '50%', width: '40px', height: '40px', color: '#ffffff', fontSize: '24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>×</button>

            <div style={{ position: 'relative', width: '100%', paddingTop: '56.25%', backgroundColor: '#000' }}>
              <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${selectedVideo.videoId}?autoplay=1`} title={selectedVideo.title} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }} />
            </div>

            <div style={{ padding: '25px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                <span style={{ fontSize: '14px', color: '#df3655', fontWeight: '500' }}>{selectedVideo.date}</span>
                <span style={{ fontSize: '12px', color: '#666', fontWeight: '400' }}>{selectedVideo.channel}</span>
              </div>
              <h3 style={{ fontSize: '22px', fontWeight: '600', color: '#000000', marginBottom: '15px', lineHeight: '1.4' }}>{selectedVideo.title}</h3>
              <div style={{ fontSize: '15px', color: '#666', lineHeight: '1.8', whiteSpace: 'pre-wrap' }}>{selectedVideo.description}</div>
              <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #e8e8e8' }}>
                <a href={selectedVideo.youtubeUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', padding: '10px 20px', backgroundColor: '#df3655', color: '#ffffff', textDecoration: 'none', borderRadius: '6px', fontWeight: '600', fontSize: '14px' }}>
                  Watch on YouTube
                </a>
              </div>
            </div>
