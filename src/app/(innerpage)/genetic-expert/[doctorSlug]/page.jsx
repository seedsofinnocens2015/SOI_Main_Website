...
const isGeneticExpertDoctor = (doctor) => GENETIC_EXPERT_SLUGS.has(doctor?.newSlug);
...
export async function generateStaticParams() {
  return doctorsData
    .filter((d) => isGeneticExpertDoctor(d))
    .map((d) => ({
      doctorSlug: d.newSlug,
    }));
}

const GeneticExpertDoctorPage = async ({ params }) => {
  const { doctorSlug } = await params;
  const doctor = doctorsData.find((d) => d.newSlug === doctorSlug && isGeneticExpertDoctor(d));
  if (!doctor) notFound();
...
export default GeneticExpertDoctorPage;
